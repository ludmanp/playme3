...
@section('title', __('Teammembers'))

@section('content')

<item-list
    url-base="/api/teammembers"
    fields="id,image_id,status,title,name,position"
    table="teammembers"
    title="teammembers"
    include="image"
    :exportable="true"
    ...
    <template slot="add-button" v-if="$can('create teammembers')">
        @include('core::admin._button-create', ['module' => 'teammembers'])
    </template>
    ...
    <template slot="table-row" slot-scope="{ model, checkedModels, loading }">
        ...
        <td v-if="$can('update teammembers')"><item-list-edit-button :url="'/admin/teammembers/'+model.id+'/edit'"></item-list-edit-button></td>
        <td><item-list-status-button :model="model"></item-list-status-button></td>
        <td><item-list-position-input :model="model"></item-list-position-input></td>
        <td><img :src="model.thumb" alt="" height="27"></td>
        <td v-html="model.title_translated"></td>
        <td v-html="model.name_translated"></td>
    </template>
